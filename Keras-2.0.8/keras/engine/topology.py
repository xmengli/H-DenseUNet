...
class Container(Layer):
    ...
    #  modified by xmli
    def load_weights(self, filepath, by_name=False , by_gpu = False, two_model = False, by_flag = False):
        ...
        if by_name:
            if by_gpu:
                if two_model:
                    load_weights_from_hdf5_group_by_name_mulgpu_twomodelcombine(f, self.layers, by_flag)
                else:
                    load_weights_from_hdf5_group_by_name_mulgpu(f, self.layers)
            else:
...
#  add by xmli
def load_weights_from_hdf5_group_by_name_mulgpu(f, layers):
    """Implements name-based weight loading.
    ...
    for k, name in enumerate(layer_names):
        g = f[name]
        weight_names = [n.decode('utf8') for n in g.keys()]
        weight_names[0], weight_names[1] = weight_names[1],weight_names[0]
        weight_values = [g[weight_name] for weight_name in weight_names]
 ...
 # modified by xmli: we aim to load two parallel model into one model
def load_weights_from_hdf5_group_by_name_mulgpu_twomodelcombine(f, layers, flag):
    """Implements name-based weight loading.
    ...
    for k, name in enumerate(layer_names):
        g = f[name]
        weight_names = [n.decode('utf8') for n in g.keys()]
        if len(weight_names) == 2 or len(weight_names) ==4:
            weight_names[0], weight_names[1] = weight_names[1],weight_names[0]
        weight_values = [g[weight_name] for weight_name in weight_names]
        print (len(weight_values))
        print ('weight_names', weight_names)
